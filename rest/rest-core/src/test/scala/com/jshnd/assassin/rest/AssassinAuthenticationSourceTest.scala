...
class AssassinAuthenticationSourceTest extends FunSpec with Inside {
  ...
  describe("AssassinAuthenticationSource") {
    ...
    val validUser = new User("test@example.com", "test", Some("Test User"), "12345")
    ...
    it("Should return a user with an invalid hash when no user found") {
      ...
      inside(result.getPrincipals.asList().get(0)) {
        case User("test@example.com", "Nope", None, AssassinAuthenticationSource.WONT_MATCH_HASH) =>
      }
    }
